...
- **Customise Status Bar** — configure the reading status bar: chapter page count, book progress %, a Book/Chapter progress bar (with thickness), Book/Chapter title, battery, time-left estimate, and a clock (UTC offset, 12h/24h). Also editable from the web settings page.
...
- **Bluetooth page turner** — pair a cheap BLE remote/clicker and turn pages without touching the device. *Any* button on the remote pages forward (no mapping step; back-paging stays on the device's own buttons), which is what makes the wildly inconsistent cheap clickers work at all. Three things to know: the remote must be in **pairing mode** before it will show up in the scan; bind **Bluetooth Remote** to a button via **Reader Controls** while setting up, because Bluetooth shuts down whenever its RAM is needed elsewhere; and it **does not come back on by itself after a sync** — re-enable it manually. See the [User Guide](USER_GUIDE.md#11-bluetooth-page-turner).
