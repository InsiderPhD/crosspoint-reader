...
void ReadingHeatmapActivity::loop() {
  ...
  buttonNavigator.onPressAndContinuous({MappedInputManager::Button::Up}, [this] { moveSelection(-7); });
  buttonNavigator.onPressAndContinuous({MappedInputManager::Button::Down}, [this] { moveSelection(7); });
}
...
void ReadingHeatmapActivity::render(RenderLock&&) {
  ...
  HeaderDateUtils::drawHeaderWithDate(renderer, tr(STR_READING_HEATMAP), "3/3");
  ...
  const std::string bestDayValue =
      monthSummary.bestDayOfMonth > 0
          ...
          : ReadingStatsAnalytics::formatDurationHm(monthSummary.bestDayReadingMs);
    drawMetricRow(renderer, Rect{sidePadding, summaryTop, contentWidth, SUMMARY_ROW_HEIGHT}, RecentIcon, tr(STR_MONTH_TOTAL),
