...
bool extractChunkSlice(HalFile& file, uint32_t compressedOffset, uint32_t compressedSize, uint32_t discardSize,
  ...
  // init(true) needs a 32KB CONTIGUOUS window, which is the allocation that
  // fails first on the C3 — after one BLE cycle the largest free block sits near
  // 25KB for the rest of the session. It resolves that itself: if a caller holds
  // an InflateScratchLease, it borrows the framebuffer (contiguous by
  // construction, so BLE fragmentation cannot reach it) and allocates nothing.
  // Every lookup path takes that lease before calling in — see
  // DictionaryLookup::run(). Without one this falls back to malloc, which is
  // correct on a PSRAM board and merely fallible on the C3.
  //
  // Upstream pre-allocated the window BEFORE this ~3KB ChunkSource, because
  // carving the small block first once left the window 12 bytes short
  // (largest=32756 against need=32768). Under the lease there is no window
  // allocation to order against, so that hazard only returns on the malloc
  // fallback — where a failure is a clean LowMemory, not a crash.
  if (!src->reader.init(true)) return fail(ExtractError::LowMemory);
  // init() leaves source/source_limit null, so the very first byte already comes
  // through the callback. Set it after, which resets state.
