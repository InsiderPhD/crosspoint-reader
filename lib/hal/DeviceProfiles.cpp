...
void savePerDeviceProfilesToStorage() {
  ...
  for (const auto& entry : perDeviceProfiles) {
    ...
    char line[96];
    snprintf(line, sizeof(line), "%s,%u,%u,%u,%u,%u,%u\n", addr.c_str(), static_cast<unsigned>(profile.pageUpCode),
             static_cast<unsigned>(profile.pageDownCode), static_cast<unsigned>(profile.reportByteIndex),
             static_cast<unsigned>(profile.pageUpIndex), static_cast<unsigned>(profile.pageDownIndex),
             static_cast<unsigned>(profile.oneButtonMode ? 1 : 0));
    all += line;
  }
...
void loadPerDeviceProfilesFromStorage() {
  ...
  while (start < content.length()) {
    ...
    if (!line.isEmpty()) {
      ...
      unsigned int upIndex = 0xFF;
      unsigned int downIndex = 0xFF;
      unsigned int oneButton = 0;
      const int parsed = sscanf(line.c_str(), "%31[^,],%u,%u,%u,%u,%u,%u", addrBuf, &up, &down, &reportIndex, &upIndex,
                                &downIndex, &oneButton);
      if (parsed >= 4 && up <= 0xFF && down <= 0xFF && (up != 0 || down != 0) && reportIndex <= 0xFF) {
        if (parsed < 6 || upIndex > 0xFF || downIndex > 0xFF) {
          upIndex = 0xFF;  // legacy line: keycode-only matching
          downIndex = 0xFF;
        }
        if (parsed < 7) {
          oneButton = 0;
        ...
        DeviceProfiles::DeviceProfile profile = {"Custom BLE Remote",
                                                 ...
                                                 false,
                                                 static_cast<uint8_t>(upIndex),
                                                 static_cast<uint8_t>(downIndex),
                                                 oneButton != 0};
...
void loadCustomProfileFromStorage() {
  ...
  unsigned int oneButton = 0;
  const int parsed =
      sscanf(content.c_str(), "%u,%u,%u,%u,%u,%u", &up, &down, &reportIndex, &upIndex, &downIndex, &oneButton);
  if (parsed >= 2 && up <= 0xFF && down <= 0xFF && (up != 0 || down != 0) && reportIndex <= 0xFF) {
    ...
    const bool haveIndexes = (parsed >= 5 && upIndex <= 0xFF && downIndex <= 0xFF);
    customProfile.pageUpIndex = haveIndexes ? static_cast<uint8_t>(upIndex) : 0xFF;
    customProfile.pageDownIndex = haveIndexes ? static_cast<uint8_t>(downIndex) : 0xFF;
    customProfile.oneButtonMode = (parsed >= 6 && oneButton != 0);
    LOG_INF("DEV", "Loaded custom BLE profile: up=0x%02X down=0x%02X idx=%u upIdx=%u dnIdx=%u",
            customProfile.pageUpCode, customProfile.pageDownCode, static_cast<unsigned>(customProfile.reportByteIndex),
            static_cast<unsigned>(customProfile.pageUpIndex), static_cast<unsigned>(customProfile.pageDownIndex));
  }
}
}  // namespace
...
const DeviceProfile* findDeviceProfile(const char* macAddress, const char* deviceName) {
  ...
  if (deviceName && strlen(deviceName) > 0) {
    for (int i = 0; i < KNOWN_DEVICES_COUNT; i++) {
      ...
      if (strstr(deviceName, "Game") || strstr(deviceName, "game") || strstr(deviceName, "GAME")) {
        if (strstr(deviceName, "Brick") || strstr(deviceName, "brick") || strstr(deviceName, "BRICK")) {
      ...
      if (strstr(deviceName, "IINE") || strstr(deviceName, "iine") || strstr(deviceName, "IINE_control") ||
          strstr(deviceName, "iine_control")) {
      ...
      if (strstr(deviceName, "MINI") || strstr(deviceName, "mini") || strstr(deviceName, "Mini")) {
        if (strstr(deviceName, "KEYBOARD") || strstr(deviceName, "keyboard") || strstr(deviceName, "Keyboard")) {
...
const DeviceProfile* getCustomProfile() {
  ...
  // A profile with a single mapped direction is valid (one-button remotes).
...
void setCustomProfile(uint8_t pageUpCode, uint8_t pageDownCode, uint8_t reportByteIndex, uint8_t pageUpIndex,
                      uint8_t pageDownIndex, bool oneButtonMode) {
  customProfile.pageUpCode = pageUpCode;
  customProfile.pageDownCode = pageDownCode;
  customProfile.reportByteIndex = reportByteIndex;
  customProfile.pageUpIndex = pageUpIndex;
  customProfile.pageDownIndex = pageDownIndex;
  customProfile.oneButtonMode = oneButtonMode;
  customProfileLoaded = true;

  Storage.mkdir("/.crosspoint");
  char buf[48];
  snprintf(buf, sizeof(buf), "%u,%u,%u,%u,%u,%u", static_cast<unsigned>(pageUpCode),
           static_cast<unsigned>(pageDownCode), static_cast<unsigned>(reportByteIndex),
           static_cast<unsigned>(pageUpIndex), static_cast<unsigned>(pageDownIndex),
           static_cast<unsigned>(oneButtonMode ? 1 : 0));
  Storage.writeFile(CUSTOM_PROFILE_FILE, buf);
  LOG_INF("DEV", "Custom profile set: up=0x%02X down=0x%02X idx=%u upIdx=%u dnIdx=%u", pageUpCode, pageDownCode,
          static_cast<unsigned>(reportByteIndex), static_cast<unsigned>(pageUpIndex),
          static_cast<unsigned>(pageDownIndex));
}

void setCustomProfileForDevice(const std::string& macAddress, uint8_t pageUpCode, uint8_t pageDownCode,
                               uint8_t reportByteIndex, uint8_t pageUpIndex, uint8_t pageDownIndex,
                               bool oneButtonMode) {
  loadPerDeviceProfilesFromStorage();
  DeviceProfile profile = {"Custom BLE Remote", nullptr, pageUpCode,  pageDownCode,  false,
                           reportByteIndex,     false,   pageUpIndex, pageDownIndex, oneButtonMode};
  perDeviceProfiles[normalizeAddress(macAddress)] = profile;
  savePerDeviceProfilesToStorage();
  LOG_INF("DEV", "Saved per-device BLE profile for %s: up=0x%02X down=0x%02X idx=%u upIdx=%u dnIdx=%u oneBtn=%d",
          macAddress.c_str(), pageUpCode, pageDownCode, static_cast<unsigned>(reportByteIndex),
          static_cast<unsigned>(pageUpIndex), static_cast<unsigned>(pageDownIndex), oneButtonMode);
}
...
void clearCustomProfile() {
  ...
  customProfile.pageUpIndex = 0xFF;
  customProfile.pageDownIndex = 0xFF;
  customProfile.oneButtonMode = false;
  customProfileLoaded = true;
