...
bool Epub::generateThumbBmp(int height) const {
  ...
  if (Storage.exists(thumbPath.c_str())) {
    ...
    // artwork arriving: a cover sidecar can be written long after the book was
    // first listed, and the sentinel would hide it forever.
    // Cheap to check -- one stat on a file we already know is there.
    FsFile existing;
  ...
  if (coverImageHref.empty()) {
    ...
    // Falls through to the sidecar: the OPF can name a cover whose bytes do not
    // decode.
