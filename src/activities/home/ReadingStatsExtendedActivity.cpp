...
constexpr int SUMMARY_GAP = 4;
constexpr int RECENT_CARD_HEIGHT = 30;
...
void drawMetricRow(GfxRenderer& renderer, const Rect& rect, const uint8_t* icon, const char* label,
  ...
  renderer.drawIcon(icon, rect.x, rect.y + 2, iconSize, iconSize);
  renderer.drawText(UI_10_FONT_ID, rect.x + iconSize + 8, rect.y + 1, label);
  const int valueWidth = renderer.getTextWidth(UI_10_FONT_ID, value.c_str(), EpdFontFamily::BOLD);
  renderer.drawText(UI_10_FONT_ID, rect.x + rect.width - valueWidth, rect.y + 1, value.c_str(), true,
                    EpdFontFamily::BOLD);
...
int getMaxScrollOffset(const GfxRenderer& renderer, const ThemeMetrics& metrics) {
  ...
  const int recentTop = summaryTop + SUMMARY_ROW_HEIGHT * 6 + SUMMARY_GAP * 2 + metrics.verticalSpacing;
...
void ReadingStatsExtendedActivity::loop() {
  ...
  if ((mappedInput.wasReleased(MappedInputManager::Button::Left) ||
      ...
      mappedInput.getHeldTime() >= PAGE_SWITCH_LONG_PRESS_MS) {
    activityManager.replaceActivity(std::make_unique<ReadingHeatmapActivity>(renderer, mappedInput));
    return;
  }
...
void ReadingStatsExtendedActivity::render(RenderLock&&) {
  ...
  GUI.drawHeader(renderer, Rect{0, metrics.topPadding, pageWidth, metrics.headerHeight}, tr(STR_READING_STATS),
    ...
    drawMetricRow(renderer, Rect{sidePadding, summaryTop, contentWidth, SUMMARY_ROW_HEIGHT}, RecentIcon, tr(STR_STREAK),
    ...
    drawMetricRow(renderer, Rect{sidePadding, summaryTop + SUMMARY_ROW_HEIGHT, contentWidth, SUMMARY_ROW_HEIGHT},
         Check24Icon, tr(STR_MAX_STREAK), std::to_string(READING_STATS.getMaxStreakDays()));
    drawMetricRow(renderer, Rect{sidePadding, summaryTop + SUMMARY_ROW_HEIGHT * 2, contentWidth, SUMMARY_ROW_HEIGHT},
         Book24Icon, tr(STR_DAILY_GOAL), dailyGoalValue);
    drawMetricRow(renderer, Rect{sidePadding, summaryTop + SUMMARY_ROW_HEIGHT * 3, contentWidth, SUMMARY_ROW_HEIGHT},
         RecentIcon, tr(STR_READING_TIME), ReadingStatsAnalytics::formatDurationHm(READING_STATS.getTotalReadingMs()));
    drawMetricRow(renderer, Rect{sidePadding, summaryTop + SUMMARY_ROW_HEIGHT * 4, contentWidth, SUMMARY_ROW_HEIGHT},
         Check24Icon, tr(STR_BOOKS_FINISHED), std::to_string(READING_STATS.getBooksFinishedCount()));
    drawMetricRow(renderer, Rect{sidePadding, summaryTop + SUMMARY_ROW_HEIGHT * 5, contentWidth, SUMMARY_ROW_HEIGHT},
         Files24Icon, tr(STR_BOOKS_STARTED), std::to_string(READING_STATS.getBooksStartedCount()));

    drawMetricRow(renderer, Rect{sidePadding, recentTop, contentWidth, RECENT_CARD_HEIGHT}, RecentIcon, "7D", last7DaysValue);
    drawMetricRow(renderer, Rect{sidePadding, recentTop + RECENT_CARD_HEIGHT, contentWidth, RECENT_CARD_HEIGHT}, RecentIcon,
         "30D", last30DaysValue);
