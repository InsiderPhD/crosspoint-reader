...
// Set for the lifetime of the screen so the ESP-NOW receive callback, which is
// a plain C function pointer with no user context, can reach the activity.
...
bool parsePosition(const uint8_t* data, const size_t len, NearbyBookPositionSyncActivity::CompactPosition& out) {
  ...
  remaining--;
  if (anchorLen > NearbyBookPositionSyncActivity::MAX_ANCHOR_BYTES || remaining < anchorLen + 1u) return false;
...
bool NearbyBookPositionSyncActivity::prepareLocalPosition() {
  ...
  // A long XPath would push the packet past ESP-NOW's 250-byte limit; the
  // percentage plus the paragraph/list hints still resolve the page without it.
...
void NearbyBookPositionSyncActivity::onEnter() {
  ...
  // No device-name setting in this fork, so name the reader after its board and
  // the tail of its MAC -- enough to tell two readers apart on the peer's screen.
  // HalGPIO::DeviceType has no X4 Pro member (that board is a separate binary),
  ...
  snprintf(name, sizeof(name), "%s %s", board, bytesToHex(localDeviceMac_.data() + 3, 3).c_str());
  localDeviceName_ = name;
...
void NearbyBookPositionSyncActivity::onExit() {
  ...
  endEspNow();
  if (shouldRestart) {
    // The WiFi session fragments the heap past the point where the reader can
    // rebuild a section in place. Reboot straight back into the book -- which
    // also picks up the position applyPeerPosition() just wrote.
    delay(30);
...
void NearbyBookPositionSyncActivity::loop() {
  ...
  // Release, not press, throughout: a Full Touch action-bar tap injects both
  // halves, and acting on the press would leave the release to land in the
  // reader we return to.
...
void NearbyBookPositionSyncActivity::endEspNow() {
  // Clear the callback's handle first: it runs on the WiFi task and would
  // otherwise still be able to reach this object while it is being torn down.
...
void NearbyBookPositionSyncActivity::enqueueEspNowPacket(const uint8_t* sourceMac, const uint8_t* data,
  ...
  if (packetType == PacketType::HELLO) {
    ...
    if (payloadLength < MIN_DEVICE_NAME_BYTES || payloadLength > MAX_DEVICE_NAME_BYTES) return;
...
void NearbyBookPositionSyncActivity::handleEvent(const SyncEvent& event) {
  // A screen sitting in ERROR (usually a 15s timeout) or SYNCED must still be
  // recruitable by a peer that starts sharing afterwards -- refusing every
  // packet there meant a reader that had timed out could only be brought back
  // by pressing Share on it a second time.
  //
  // What it must NOT do is let a stray packet tear down a comparison the user is
  // reading, so the two error paths below stay quiet while one is on screen.
  const bool showingComparison = state_ == State::SHOWING_RESULT;

  if (event.type == PacketType::INVALID) {
    if (!showingComparison) setError(tr(STR_NEARBY_POSITION_VERSION_MISMATCH));
    return;
  }

  // NAME doubles as the "I'm here" reply to a HELLO, so it is what tells a
  // sender which peer to unicast its position to.
  ...
  if (event.type != PacketType::ACK &&
    ...
    if (!showingComparison) setError(tr(STR_NEARBY_POSITION_BOOK_MISMATCH));
    return;
  }

  // The other reader pressed Share first: fall into receive mode from READY.
  const bool startingPassiveSync =
      state_ != State::DISCOVERING && state_ != State::SYNCING && state_ != State::SHOWING_RESULT;
  ...
  if (event.type == PacketType::POSITION) {
    // Both readers pressed Share. Nothing stops that, and with each side
    // discarding the other's position both sat there until they timed out --
    // which is what made applying a position take two attempts. Break the tie on
    // device MAC so exactly one side yields; both compute the same answer from
    // the same two addresses. A peer that has already acked our position wins
    // outright: that exchange is done and must not be undone here.
    if (sourceMode_) {
      if (localPositionAcked_ || localDeviceMac_ < event.deviceMac) return;
      LOG_DBG(LOG_TAG, "Both readers shared; yielding to %s", peerId_.c_str());
      sourceMode_ = false;
    }
    peerPosition_ = event.position;
    if (!mapPeerPosition()) return;
    peerPositionReceived_ = true;
    // Acknowledge receipt straight away: the sender stops retrying and reports
    // "Position shared" rather than sitting in its 15s window waiting for the
    // reader on the other end to get round to pressing Select.
    sendAck(peerSourceMac_.data());
  ...
  if (event.type == PacketType::ACK) {
    ...
    if (sourceMode_) setState(State::SYNCED);
  }
}
...
bool NearbyBookPositionSyncActivity::sendPacket(const PacketType type, const uint8_t* peerMac) {
  ...
  // 250 bytes, on the stack: the ESP-NOW payload ceiling, not a guess.
  ...
  if (type == PacketType::HELLO) {
    ...
    payloadLength = std::min(localDeviceName_.size(), MAX_DEVICE_NAME_BYTES);
    if (payloadLength < MIN_DEVICE_NAME_BYTES) return false;
    memcpy(payload, localDeviceName_.data(), payloadLength);
  }
...
bool NearbyBookPositionSyncActivity::sendHello(const uint8_t* peerMac) {
  lastHelloMs_ = millis();
  return sendPacket(PacketType::HELLO, peerMac);
}
...
bool NearbyBookPositionSyncActivity::mapPeerPosition() {
  ...
  // Nothing usable came back from the XPath: fall back to the raw spine/page the
  // peer sent. Both readers laid the book out with their own font and margins,
  // so this is only a starting point -- the refinement below is what makes it land.
  ...
  if (peerCrossPoint_.hasLiIndex || peerCrossPoint_.xpathAnchorId[0] != '\0' || peerCrossPoint_.hasParagraphIndex) {
    // Built from the cached layout for that spine item, so this resolves the
    // peer's paragraph to a page in OUR pagination.
  ...
  peerCrossPoint_.totalPages = std::max(1, std::max(peerCrossPoint_.totalPages, peerCrossPoint_.pageNumber + 1));
  return true;
}
...
void NearbyBookPositionSyncActivity::updateSyncProgress() {
  ...
  // One retry timer for the position, so a dropped packet re-sends without
  // stacking up a fresh send on every loop iteration.
...
void NearbyBookPositionSyncActivity::render(RenderLock&&) {
  ...
  if (state_ == State::SHOWING_RESULT) {
    renderComparison();
    const auto labels = mappedInput.mapLabels(tr(STR_BACK), sourceMode_ ? "" : tr(STR_SELECT), "", "");
  ...
  switch (state_) {
    ...
    case State::READY:
      ...
      detail = std::string(tr(STR_DEVICE_NAME)) + ": " + localDeviceName_;
    ...
    case State::SYNCING:
      ...
      detail = std::string(tr(STR_DEVICE_NAME)) + ": " + (peerName_.empty() ? peerId_ : peerName_);
      break;
    case State::SYNCED:
  ...
  renderStatus(primary, detail, detailSecondary);

  // Share is offered again after a success or a failure so a mistimed attempt
  // can be retried without leaving and re-entering the screen.
  const bool canShare = state_ == State::READY || state_ == State::SYNCED || state_ == State::ERROR;
  const auto labels = mappedInput.mapLabels(tr(STR_BACK), canShare ? tr(STR_NEARBY_POSITION_SHARE_BUTTON) : "", "", "");
  GUI.drawButtonHints(renderer, labels.btn1, labels.btn2, labels.btn3, labels.btn4);
  if (SETTINGS.darkMode) renderer.invertScreen();
  renderer.displayBuffer();
}

void NearbyBookPositionSyncActivity::renderStatus(const char* primary, const std::string& detail,
  ...
  renderer.drawCenteredText(UI_10_FONT_ID, y, primary, true, EpdFontFamily::BOLD);
  y += lineHeight + metrics.verticalSpacing;

  if (!detail.empty()) {
    for (const auto& line : renderer.wrappedText(SMALL_FONT_ID, detail.c_str(), contentWidth, 3)) {
  ...
  if (!detailSecondary.empty()) {
    renderer.drawCenteredText(SMALL_FONT_ID, y, detailSecondary.c_str());
    y += smallLineHeight + metrics.verticalSpacing;
  }
  if (state_ == State::READY) {
    for (const auto& line : renderer.wrappedText(SMALL_FONT_ID, tr(STR_NEARBY_POSITION_READY_HINT), contentWidth, 2)) {
      renderer.drawCenteredText(SMALL_FONT_ID, y, line.c_str());
      y += smallLineHeight;
    }
  }
...
void NearbyBookPositionSyncActivity::renderComparison() const {
  ...
  renderer.drawCenteredText(UI_10_FONT_ID, y, tr(STR_NEARBY_POSITION_FOUND), true, EpdFontFamily::BOLD);
  ...
  char buf[128];
  renderer.drawText(UI_10_FONT_ID, pad, y, tr(STR_NEARBY_LABEL), true);
  y += lineHeight;
  snprintf(buf, sizeof(buf), "  %s", peerChapter.c_str());
  renderer.drawText(UI_10_FONT_ID, pad, y, buf);
  y += lineHeight;
  snprintf(buf, sizeof(buf), tr(STR_PAGE_OVERALL_FORMAT), peerCrossPoint_.pageNumber + 1,
           qToPercentage(peerPosition_.percentageQ) * 100.0f);
  renderer.drawText(UI_10_FONT_ID, pad, y, buf);
  y += lineHeight;
  if (!peerName_.empty()) {
    snprintf(buf, sizeof(buf), tr(STR_DEVICE_FROM_FORMAT), peerName_.c_str());
    renderer.drawText(UI_10_FONT_ID, pad, y, buf);
    y += lineHeight;
  }
  y += metrics.verticalSpacing;

  renderer.drawText(UI_10_FONT_ID, pad, y, tr(STR_LOCAL_LABEL), true);
  y += lineHeight;
  snprintf(buf, sizeof(buf), "  %s", localChapter.c_str());
  renderer.drawText(UI_10_FONT_ID, pad, y, buf);
  y += lineHeight;
  snprintf(buf, sizeof(buf), tr(STR_PAGE_TOTAL_OVERALL_FORMAT), currentPage_ + 1, totalPagesInSpine_,
           qToPercentage(localPosition_.percentageQ) * 100.0f);
  renderer.drawText(UI_10_FONT_ID, pad, y, buf);
  y += lineHeight + metrics.verticalSpacing * 2;

  // Only the receiving side can act on the comparison; the sender is looking at
  // the same screen purely as confirmation of what it sent.
  if (!sourceMode_) {
    renderer.fillRect(0, y - 2, pageWidth - 1, lineHeight + 4);
    renderer.drawText(UI_10_FONT_ID, pad, y, tr(STR_APPLY_NEARBY_POSITION), false);
  }
}
