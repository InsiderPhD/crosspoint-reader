...
#   endif
        /* CrossPoint (vendored patch): this firmware is a single-connection HID
         * client on a 320KB-heap device. Trim the controller's RAM before init:
         * - ble_max_act: 1 connection + 1 scan + 1 margin (library computes up
         *   to 5 from the sdkconfig's 3-connection host config).
         * - normal_adv_size: duplicate-scan cache 100 -> 20 entries (the app
         *   deduplicates scan results itself).
         * - ble_50_feat_supp: the host is compiled legacy-only (no
         *   CONFIG_BT_NIMBLE_EXT_ADV), so 5.0 LL feature contexts are unused. */
        bt_cfg.ble_max_act      = 3;
        bt_cfg.normal_adv_size  = 20;
        bt_cfg.ble_50_feat_supp = false;
