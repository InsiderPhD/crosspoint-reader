...
/* CrossPoint (vendored patch): halve the mbuf pools. The framework sdkconfig
 * bakes 12+24 blocks sized for multi-connection peripherals; this firmware is
 * a single-connection HID client whose traffic is <=10-byte notifications.
 * Patched here (single allocation site) rather than via CONFIG macros, which
 * the framework's sdkconfig.h overrides in every TU. */
#define OS_MSYS_1_BLOCK_COUNT (6)
#define OS_MSYS_1_BLOCK_SIZE MYNEWT_VAL(MSYS_1_BLOCK_SIZE)
#define OS_MSYS_2_BLOCK_COUNT (12)
