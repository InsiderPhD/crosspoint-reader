...
bool buildDictionaryHtmlPages(GfxRenderer& renderer, const std::string& definition, const uint16_t viewportWidth,
  ...
  size_t retainedElements = 0;
  {
    ...
    auto parser = makeUniqueNoThrow<ChapterHtmlSlimParser>(
        nullptr, tmpPath, renderer, SETTINGS.getReaderFontId(), SETTINGS.getCodeFontId(),
        SETTINGS.getReaderLineCompression(), SETTINGS.extraParagraphSpacing, SETTINGS.paragraphAlignment, viewportWidth,
        viewportHeight, SETTINGS.hyphenationEnabled,
        [&pagesOut, &resourceLimitHit, &retainedElements, &limitReason](std::unique_ptr<Page> page, uint16_t,
                                                                        uint16_t) {
        ...
        // A definition is not a chapter: there is no anchor target to jump to and
        // no footnote list screen behind it, so collecting footnotes would only
        // spend heap on entries nothing can reach.
        /*footnoteDisplayOnPage=*/false,
        // Bionic reading is a reading aid, not a book property — if it is on for
        // the chapter it should be on for the definition too.
        /*bionicReadingEnabled=*/SETTINGS.bionicReading != 0);
    if (!parser) {
