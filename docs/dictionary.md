...
- Open the reader menu (**Confirm**) and choose **Look Up**. The row appears only once a dictionary is selected.
- Or bind **Look Up** to any reader button in **Settings → Reader → Reader Controls** — it is available for every short press, long press and tap zone.

On the X3 and X4 (ESP32-C3), **Bluetooth must be off**. A lookup needs a 32KB
contiguous inflate window plus the definition buffer and, for styled
dictionaries, a laid-out page set — all from the same heap the BLE stack is
holding around 50KB of. Rather than risk failing mid-lookup, the reader refuses
with *"Turn off Bluetooth to look up words"*. Toggle the page-turner off from the
reader menu, look the word up, and turn it back on. The X4 Pro has PSRAM and
radio coexistence, so it has no such restriction.
...
On touch devices, tapping a word moves the highlight to it and tapping it again looks it up; a tap-and-hold looks it up directly. This is the same word selector the clippings feature uses, in a single-word mode.
...
On the C3 boards the styled path declines more often than it does upstream, because the reader already holds more at that moment. The fallback is the same definition in plain text, not a failed lookup.
