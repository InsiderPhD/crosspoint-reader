...
Plenty of sideloaded EPUBs carry no artwork at all, or name a cover that does not decode, and sat in the library as blank tiles. Whoever put the book on the card can now fetch the cover from the catalogue that has it and drop it next to the book — `/Books/Title.epub` → `/Books/Title.jpg` — and the thumbnail builder falls back to that file (`.jpg`, `.jpeg`, `.png`) once the embedded cover has failed. A book that does carry usable artwork keeps using its own.

The zero-byte "no cover here" sentinel needed a way out for this to work. It exists so a book with no artwork is not re-examined on every paint, but a sidecar can arrive long after the book was first listed, and the sentinel would have hidden it forever. An empty thumb now costs one stat and a sidecar re-check rather than being final.
