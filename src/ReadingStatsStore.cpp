...
uint64_t getDailyReadingGoalMs() { return SETTINGS.getDailyGoalMs(); }
...
bool ReadingStatsStore::hasTimeOfDayData() const {
  for (const uint64_t bucket : timeOfDayMs) {
    if (bucket > 0) {
      return true;
    }
  }
...
void ReadingStatsStore::refreshLegacyCounters() {
  totalReadingTimeSeconds = static_cast<uint32_t>(std::min<uint64_t>(getTotalReadingMs() / 1000ULL, UINT32_MAX));
  booksFinished = getBooksFinishedCount();
  totalSessions = static_cast<uint32_t>(std::min<size_t>(sessionLog.size(), static_cast<size_t>(UINT32_MAX)));
}
...
void ReadingStatsStore::appendSessionLogEntry(const uint32_t dayOrdinal, const uint32_t sessionMs,
                                              const std::string& bookId, const uint8_t endProgressPercent) {
  ...
  entry.bookId = bookId;
  entry.endProgressPercent = endProgressPercent;
...
uint32_t ReadingStatsStore::resolveCurrentMonthReferenceDayOrdinal() const {
  // Do not attempt NTP sync here: loadFromFile() runs during boot before
  // lwIP/tcpip is guaranteed to be initialized, which can trigger
  // `tcpip_callback ... (Invalid mbox)` assert and reboot loops.
  return getReferenceDayOrdinal();
}

bool ReadingStatsStore::pruneToCurrentMonth(const uint32_t referenceDayOrdinal) {
  ...
  const uint32_t futureLimitOrdinal =
      isClockValid(TimeUtils::getCurrentValidTimestamp()) ? referenceDayOrdinal + STATS_FUTURE_GRACE_DAYS : UINT32_MAX;
  ...
  auto pruneDaysVector = [&keepDay, &changed](std::vector<ReadingDayStats>& days) {
    const size_t before = days.size();
    days.erase(
        std::remove_if(days.begin(), days.end(),
                       [&keepDay](const ReadingDayStats& stats) { return !keepDay(stats) || stats.readingMs == 0; }),
        days.end());
    if (days.size() != before) {
      changed = true;
    }
  ...
  pruneDaysVector(legacyReadingDays);

  for (auto& book : books) {
    pruneDaysVector(book.readingDays);

    uint64_t totalReadingMs = 0;
    for (const auto& dayStats : book.readingDays) {
      totalReadingMs += dayStats.readingMs;
    }
    if (book.totalReadingMs != totalReadingMs) {
      book.totalReadingMs = totalReadingMs;
      changed = true;
    }
    ...
    if (book.totalReadingMs == 0) {
      ...
      if (book.lastSessionMs != 0) {
        book.lastSessionMs = 0;
        changed = true;
      }
  ...
  if (!books.empty()) {
    const size_t activeBookIndex = activeSession.active ? activeSession.bookIndex : books.size();
    std::vector<ReadingBookStats> prunedBooks;
    prunedBooks.reserve(books.size());

    bool activeSessionStillValid = !activeSession.active;
    size_t newActiveBookIndex = 0;

    for (size_t index = 0; index < books.size(); ++index) {
      const bool keepBook = !books[index].readingDays.empty() || (activeSession.active && index == activeBookIndex);
      if (!keepBook) {
        changed = true;
        continue;
      }

      if (activeSession.active && index == activeBookIndex) {
        activeSessionStillValid = true;
        newActiveBookIndex = prunedBooks.size();
      }
      prunedBooks.push_back(std::move(books[index]));
    }

    books = std::move(prunedBooks);
    if (activeSession.active) {
      if (activeSessionStillValid) {
        activeSession.bookIndex = newActiveBookIndex;
      } else {
        activeSession = {};
        lastSessionSnapshot = {};
      }
  ...
  sessionLog.erase(std::remove_if(sessionLog.begin(), sessionLog.end(),
                                  [retentionStartOrdinal, futureLimitOrdinal](const ReadingSessionLogEntry& entry) {
                                    ...
                                    // (Total count is still bounded by MAX_SESSION_LOG_ENTRIES in
...
void ReadingStatsStore::flushActiveSessionToBuckets() {
  ...
  // Time-of-day is credited to the bucket the clock is in right now, for
  // whatever accrued since the last checkpoint. Heartbeats are a minute apart,
  // so a session that spans a boundary splits across the two buckets within a
  // minute of the real crossing. Done before the accumulated == bucketed early
  // return below so it tracks its own watermark.
  uint8_t localHour = 0;
  if (activeSession.accumulatedMs > activeSession.todBucketedMs &&
      TimeUtils::getLocalHour(sessionTimestamp, localHour)) {
    const size_t bucket = static_cast<size_t>(readingTimeBucketForHour(localHour));
    timeOfDayMs[bucket] += activeSession.accumulatedMs - activeSession.todBucketedMs;
    activeSession.todBucketedMs = activeSession.accumulatedMs;
  }

  if (activeSession.accumulatedMs == activeSession.bucketedMs) {
    return;
  }
...
void ReadingStatsStore::endSession() {
  ...
  if (countedSession) {
    ...
    appendSessionLogEntry(dayOrdinal, sessionMs, book.bookId, book.lastProgressPercent);
  }
  ...
  // The session is on SD; mirror the book's progress to Hardcover in the
  // background. Skipped for a glance too short to count, unless it finished the
  // book. Looked up by path because the prune above may have rewritten `books`.
  if (lastSessionSnapshot.counted || lastSessionSnapshot.completedThisSession) {
    if (const ReadingBookStats* synced = findBook(lastSessionSnapshot.path)) {
      HardcoverSync::onSessionWritten(*synced);
    }
  }
#endif
}
...
bool ReadingStatsStore::editSessionDate(const size_t index, const uint32_t newDayOrdinal) {
  ...
  if (oldDay != 0) {
    auto it =
        std::lower_bound(book->readingDays.begin(), book->readingDays.end(), oldDay,
                         [](const ReadingDayStats& day, const uint32_t ordinal) { return day.dayOrdinal < ordinal; });
...
void ReadingStatsStore::reset() {
  ...
  readingDays.clear();
  sessionLog.clear();
  for (uint64_t& bucket : timeOfDayMs) {
    bucket = 0;
  }
  activeSession = {};
  lastSessionSnapshot = {};
...
bool ReadingStatsStore::importFromFile(const std::string& path) {
  ...
  removeIgnoredBooks();
  rebuildAggregatedReadingDays();
  const bool prunedToCurrentMonth = pruneToCurrentMonth(resolveCurrentMonthReferenceDayOrdinal());
  const uint32_t latestKnownTimestamp = getLatestKnownTimestamp();
...
bool ReadingStatsStore::loadFromFile() {
  ...
  if (loaded) {
    ...
    invalidateSummaryCache();
    refreshLegacyCounters();
    if (needsSave || prunedToCurrentMonth) {
      markDirty();
      saveToFile();
    } else {
      dirty = false;
...
void ReadingStatsStore::addReadingTime(const uint32_t seconds) {
  // Compatibility fallback when old call sites are still present.
  const uint64_t addedMs = static_cast<uint64_t>(seconds) * 1000ULL;
  if (addedMs == 0) {
    return;
  }
  const uint32_t referenceTimestamp = getReferenceTimestamp(TimeUtils::getAuthoritativeTimestamp());
  auto& day = getOrCreateReadingDay(referenceTimestamp);
  day.readingMs += addedMs;
  markDirty();
  refreshLegacyCounters();
}

void ReadingStatsStore::addPageTurns(const uint32_t count) { totalPagesRead += count; }

void ReadingStatsStore::addBookFinished() { booksFinished = std::min<uint32_t>(UINT32_MAX, booksFinished + 1); }

void ReadingStatsStore::addSession() { totalSessions = std::min<uint32_t>(UINT32_MAX, totalSessions + 1); }
