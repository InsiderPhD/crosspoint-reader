...
// Internal sentinel codes emitted by the report parser for index-locked learned
// profiles (remotes where every button sends the SAME keycode and the button
// identity lives in which report byte carries it). Not real HID usages.
constexpr uint8_t LEARNED_BACK_CODE = 0xF8;
constexpr uint8_t LEARNED_FORWARD_CODE = 0xF9;
...
struct DeviceProfile {
  ...
  // Learned profiles only: byte position each direction's code was captured at.
  // 0xFF = not index-locked (legacy keycode-only matching). When set, a report
  // matches a direction only if report[index] == code, which disambiguates
  // remotes where every button sends the same keycode at different positions.
  uint8_t pageUpIndex = 0xFF;
  uint8_t pageDownIndex = 0xFF;
  // One-button (toggle) remotes: each press alternates between two codes.
  // When set, BOTH learned codes mean page-forward.
  bool oneButtonMode = false;
};
 ...
 * pageUpIndex/pageDownIndex lock each direction to a report byte (0xFF = keycode-only matching).
 */
void setCustomProfile(uint8_t pageUpCode, uint8_t pageDownCode, uint8_t reportByteIndex, uint8_t pageUpIndex = 0xFF,
                      uint8_t pageDownIndex = 0xFF, bool oneButtonMode = false);
...
void setCustomProfileForDevice(const std::string& macAddress, uint8_t pageUpCode, uint8_t pageDownCode,
                               uint8_t reportByteIndex, uint8_t pageUpIndex = 0xFF, uint8_t pageDownIndex = 0xFF,
                               bool oneButtonMode = false);
